...
class Communicator(doing.DoDoer):
                ...
                try:
                    ...
                    if creder.schema == QVI_SCHEMA:
                        ...
                        raise kering.ValidationError(
                            f'credential {creder.said} is of unsupported schema {creder.schema} from issuer {creder.issuer}'
                        )
            ...
            if said not in self.clients:
                ...
                if action == 'iss':  # presentation of issued credential
                    if creder.schema == QVI_SCHEMA:
                        ...
                        logger.error(
                            f'invalid credential with schema {creder.schema} said {creder.said} issuer {creder.issuer}'
                        )
                        raise kering.ValidationError('this will never happen because all credentials that get here are valid')
    ...
    def request(self, said, resource, action, actor, data):
        ...
        body = dict(action=action, actor=actor, data=data)
        ...
        client.request(method='POST', path=path, qargs=parse.parse_qs(purl.query), headers=headers, body=raw)
    ...
    def validateQualifiedvLEIIssuer(self, creder):
        ...
        if not creder.issuer == self.auth:
            ...
            raise kering.ValidationError(
                f'QVI credential not issued by known valid issuer. Expected {self.auth} found {creder.issuer}'
            )
    ...
    def validateOfficialRole(self, creder):
        ...
        if auth.sad['a']['AID'] != creder.attrib['i']:
            raise kering.ValidationError(
                ...
                f'{auth.sad["a"]["AID"]} for OOR '
                f'credential {creder.said}'
            )

        if auth.sad['a']['personLegalName'] != creder.attrib['personLegalName']:
            raise kering.ValidationError(
                f'invalid personLegalNAme {creder.attrib["personLegalName"]} for OOR credential {creder.said}'
            )

        if auth.sad['a']['officialRole'] != creder.attrib['officialRole']:
            raise kering.ValidationError(f'invalid role {creder.attrib["officialRole"]} for OOR credential {creder.said}')
    ...
    def revokePayload(self, creder):
        ...
        data = dict(
            type=type_to_name[creder.schema], schema=creder.schema, credential=creder.said, revocationTimestamp=state.dt
        )
